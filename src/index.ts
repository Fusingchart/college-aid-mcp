...
function simulateIdr(
...
): { totalPaid: number; forgiven: number; paidOffMonth: number | null } {
  const r = annualRate / 12;
  ...
  for (let month = 1; month <= forgivenessMonths; month++) {
    ...
    if (balance <= 0) return { totalPaid, forgiven: 0, paidOffMonth: month };
  }
...
server.tool(
  "estimate_loan_repayment",
  "Calculate monthly payments, total interest, and payoff timeline across repayment plans (standard 10-year, extended 25-year, income-driven with forgiveness). Works standalone or pairs naturally with get_college_details and search_by_major to turn debt numbers into concrete repayment scenarios.",
  {
  ...
  async ({ loan_amount, annual_income, interest_rate, household_size }) => {
    ...
    function dtiRating(pct: number): string {
    ...
    if (idrMonthly >= stdMonthly) {
      recommendation = `Your income is high relative to your debt — standard repayment saves the most in total interest (${fmt(stdInterest as number)} vs ${fmt(extInterest as number)} on extended). IDR would actually cost more per month here.`;
...
server.tool(
  "compare_colleges",
  "Side-by-side comparison of 2–5 colleges across admissions, cost, financial aid, and outcomes. Takes a list of school names and returns a structured table so you don't have to call get_college_details multiple times and mentally line them up.",
  {
    schools: z
      .array(z.string())
      .min(2)
  ...
  async ({ schools }) => {
    ...
    const allResults = await Promise.all(
      schools.map((name) => fetchSchoolsByName(name, SCORECARD_API_KEY, 1))
    );

    // Pair each queried name with its result (or null if not found)
    const matched: { queried: string; school: SchoolRecord | null }[] = schools.map((name, i) => ({
      queried: name,
    ...
    const g = (school: SchoolRecord, key: string) => school[key] ?? null;

    // Helper: SAT composite range string
    function satRange(s: SchoolRecord): string {
      ...
      if (m25 == null || r25 == null) return "N/A";
    ...
    const table = [
      ...
      row("**Type**",            found.map((m) => decodeOwnership(g(m.school!, "school.ownership") as number))),
      row("**Classification**",  found.map((m) => decodeCarnegie(g(m.school!, "school.carnegie_basic") as number))),
      row("**Enrollment**",      found.map((m) => g(m.school!, "latest.student.size") != null ? Number(g(m.school!, "latest.student.size")).toLocaleString() : "N/A")),

      // Admissions
      row("**Acceptance rate**", found.map((m) => fmtPct(g(m.school!, "latest.admissions.admission_rate.overall") as number))),
      ...
      // Cost
      row("**In-state tuition**",    found.map((m) => fmt(g(m.school!, "latest.cost.tuition.in_state") as number))),
      row("**Out-of-state tuition**", found.map((m) => fmt(g(m.school!, "latest.cost.tuition.out_of_state") as number))),
      row("**Avg net price**",       found.map((m) => fmt(g(m.school!, "latest.cost.avg_net_price.consumer.overall_median") as number))),
      row("**Net price ≤$30k fam.**", found.map((m) => fmt(g(m.school!, "latest.cost.net_price.consumer.by_income_level.0-30000") as number))),
      row("**Net price $75–110k fam.**", found.map((m) => fmt(g(m.school!, "latest.cost.net_price.consumer.by_income_level.75001-110000") as number))),
      row("**Net price $110k+ fam.**", found.map((m) => fmt(g(m.school!, "latest.cost.net_price.consumer.by_income_level.110001-plus") as number))),

      // Aid & debt
      row("**Median debt**",     found.map((m) => fmt(g(m.school!, "latest.aid.median_debt.completers.overall") as number))),
      row("**Pell grant rate**", found.map((m) => fmtPct(g(m.school!, "latest.aid.pell_grant_rate") as number))),

      // Outcomes
      row("**4-yr grad rate**",  found.map((m) => fmtPct(g(m.school!, "latest.completion.completion_rate_4yr_150nt") as number))),
      row("**1-yr retention**",  found.map((m) => fmtPct(g(m.school!, "latest.student.retention_rate.four_year.full_time") as number))),
      row("**10-yr earnings**",  found.map((m) => fmt(g(m.school!, "latest.earnings.10_yrs_after_entry.median") as number))),
      row("**First-gen students**", found.map((m) => fmtPct(g(m.school!, "latest.student.demographics.first_generation") as number))),
    ].join("\n");
